...
export function ClientPage({ topicName, initialData }: ClientPageProps) {
      ...
      useEffect(() => {
            socket.on("room-update", (message: string) => {
                  ...
                  data.map((newWord) => {
                        const isWordAlreayIncluded = words.some((word) => word.text === newWord.text)

                        if (isWordAlreayIncluded) {
                              setWords((prev) => {
                  ...
                  <MaxWidthWrapper className="flex flex-col items-center gap-6 pt-20">
                        ...
                        <div className="max-w-lg w-full">
                              <Label className="font-semibold tracking-tight text-lg pb-2">
                                    Here&apos;s what i thnk about {topicName}
                              </Label>

                              <div className="mt-1 flex gap-2 items-center">
                                    <Input value={input} onChange={({ target }) => setInput(target.value)} placeholder={`${topicName} is absolutely...`} />
